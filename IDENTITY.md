...
| Thuộc tính | Giá trị |
|------------|---------|
| Package | `@diepxuan/dsh-zero-trust` v0.1.0 (private, ESM, không dependencies) |
...
| Target vá | `@deepseek-ai/dsh-client-ui-settings` (2 gate scope `"host":"memory"`) + `@deepseek-ai/dsh-client-ui-settings-general` (1 gate document store) |
| Anchor resolve | `$DSH_HOME/profiles/node_modules` → resolve từ cwd → vị trí cài plugin |
| Backup | `<file>.bak-zero-trust` tạo một lần, bất biến |
...
2. Giữ nguyên tắc zero-trust: chỉ bỏ gate loopback phía client; trust fence server-side, `--trusted-host`, Cloudflare Access phải bất biến
